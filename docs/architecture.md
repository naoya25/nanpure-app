...
| **アルゴリズム** | `lib/algorithms/` — 生成・求解など **81 文字列の盤**を扱う処理。UI や `SudokuGrid` には直接依存しない。 |
...
| `lib/algorithms/` | **求解・列挙・一意解判定**など、文字列盤を対象にしたエンジン。CLI や将来のサーバー組み立てから再利用する。 |
...
- **自動問題生成**: 対角ブロック（0,4,8）のみを埋めたシードから完成盤を 1 通り求め、一意解を保つようマスを削る（実装は **`scripts/generate_sudoku.ts`**。ソルバ本体は **`lib/algorithms/`**）。CLI での保存は `scripts/create-puzzle.ts`（`npm run create-puzzle`）。サーバーからの組み立てが必要な場合は `lib/services/` が同じ生成モジュールを import してよい。**生成時の `level` は暫定固定**し、後から判定関数に差し替える。
