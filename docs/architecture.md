...
| `lib/algorithms/` | **求解・列挙・一意解判定**（文字列盤）と、**テクニックごとの 1 手検出・適用指示**（`techniques/`、DTO 入力。交差・サブセット・**基本魚（fishNN）**・**スカイスクレーパー** など）。**難易度スコア**（テクニック使用回数×希少度ウェイト、実験レポートに基づく固定係数、未解決時は上限扱い）もここに置く。CLI / サーバー / `lib/models` の窓口から再利用する。 |
...
- 2026-04-06: テクニック使用回数から 100 点満点の難易度を計算するモジュールを `lib/algorithms/` に追加（希少度は 1000 問実験レポートの「使用率(全体)」に基づく固定ウェイト、未解決はメイト級加点で 100 点）。`lib/algorithms/` のディレクトリ説明を更新。
